...
export default function AttendedForm({ onBack }: { onBack: () => void }) {
  const [messages, setMessages] = useState<Message[]>([
    { role: 'assistant', content: "Hola! Sóc l'assistent de Càritas i estic aquí per ajudar-te. Tot el que em comentes és confidencial. En quin municipi vius?" }
  ]);
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-8 pb-28 md:pb-8">
      ...
      <div className="space-y-4 mb-6">
        {messages.map((msg, i) => (
          <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            {msg.role === 'assistant' && (
              <div className="w-8 h-8 rounded-full flex items-center justify-center mr-2 mt-1" style={{ background: 'rgba(147,51,234,0.1)' }}>
        ...
        {loading && (
          <div className="flex justify-start">
            <div className="w-8 h-8 rounded-full flex items-center justify-center mr-2" style={{ background: 'rgba(147,51,234,0.1)' }}>
              <Sparkles size={14} style={{ color: '#7c3aed' }} />
            </div>
      ...
      {matchResult && (
        <div className="mb-6 rounded-2xl border-2 p-6" style={{ borderColor: '#7c3aed', background: 'rgba(147,51,234,0.02)' }}>
          <div className="flex items-center gap-2 mb-3">
            <Sparkles size={16} style={{ color: '#7c3aed' }} />
            <span className="text-xs font-semibold uppercase tracking-wider text-gray-400">Recursos trobats</span>
            <span className="ml-auto text-xs font-bold px-3 py-1 rounded-full text-white"
              style={{ background: (matchResult.urgencia === 'alta' || matchResult.urgencia === 'crítica') ? '#dc2626' : '#f59e0b' }}>
              {String(matchResult.urgencia || '').toUpperCase()}
            </span>
          </div>
          <p className="text-sm text-gray-700 mb-4">{String(matchResult.perfil_resum || '')}</p>
          {(matchResult.recursos as Array<{nom:string;unitat:string}>)?.slice(0,3).map((r, i) => (
      ...
      {!matchResult && (
        <div className="bg-white border border-gray-200 rounded-2xl p-3 shadow-sm">
          <textarea value={input} onChange={(e) => setInput(e.target.value)}
            ...
            placeholder="Escriu aquí la teva situació..." rows={2} disabled={loading}
